...
POSTGRES_URL = os.getenv("POSTGRES_URL", "jdbc:postgresql://localhost:5432/restaurant_db")
POSTGRES_USER = os.getenv("PGUSER", "restaurant")
POSTGRES_PASSWORD = os.getenv("PGPASSWORD", "restaurant")
CHECKPOINT_DIR = os.getenv("CHECKPOINT_DIR", "/tmp/spark_checkpoints/orders_weather")
...
def upsert_kpi(batch_df, batch_id):
    ...
    conn = psycopg2.connect(dbname='restaurant_db', user='airflow', password='airflow', host='localhost', port=5432)
    cur = conn.cursor()
    sql = """
...
if os.path.exists(MODEL_PATH):
    try:
        ...
        def write_preds(batch_df, batch_id):
            ...
            conn = psycopg2.connect(dbname='restaurant_db', user='airflow', password='airflow', host='localhost', port=5432)
            cur = conn.cursor()
            sql = "INSERT INTO predictions (ts, branch_id, revenue, predicted_revenue) VALUE %s"
            execute_values(cur, sql, tuples)
            conn.commit()
            cur.close()
            conn.close()
